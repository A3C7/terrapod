...
| **Terraform / OpenTofu provider** | [`terraform-provider-terrapod`](terraform-provider.md) — manage Terrapod itself as code: 26 `terrapod_*` resources + 11 data sources (workspaces, variables, roles, VCS connections, agent pools, run tasks/triggers, notifications, registry, catalog, execution hooks, …). Served per-instance from `<host>/default/terrapod`, GPG-signed |
| **AI agent integration (MCP)** | [`terrapod-mcp`](mcp.md) — an official MCP server that lets an AI agent (Claude, Cursor, …) observe and act on a Terrapod instance through typed, RBAC-checked tools. Local stdio, authed with your `tofu login` token; read-rich Observe (workspaces, runs, plan JSON, drift) + gated Act (plan/apply); one server per instance for prod/dev isolation |
...
| [Terraform provider](terraform-provider.md) | `terraform-provider-terrapod` — manage Terrapod itself as code (workspaces, variables, roles, VCS connections, agent pools, run tasks, catalog, execution hooks, …) via 26 `terrapod_*` resources + 11 data sources; served per-instance and GPG-signed |
...
| [AI Architecture Critique](architecture-critique.md) | Optional AI review of a workspace's **deployed system as it exists** (from state), across reliability/security/cost/operations — distinct from the per-run plan summary. Grounded in the Checkov/Trivy scan + native cost engine + resource graph; auto on new state version + on-demand regenerate; its own `ai_architecture` config (off by default) |
...
| [OpenBao/Vault](vault.md) | OpenBao (or HashiCorp Vault) as a variable value source: a variable holds a reference resolved at run time, so the server stays the source of truth and nothing is stored in Terrapod. OpenBao is recommended and Vault is equally supported. Static kv-v2 and dynamic engines (a fresh credential per run); Kubernetes auth by default, JWT auth for a server outside the cluster (incl. HCP Vault Dedicated), AppRole and token; multiple servers; a private CA and a path allow-list per instance; an admin status page per instance, and a reference **Check** that never mints a credential, for diagnosing a variable that won't resolve |
