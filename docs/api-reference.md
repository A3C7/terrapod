...
`file` is refused with `422` together with `hcl`, on a `static` value source,
with any unknown key (`mode` is reserved), when more than one of `field`,
